...
pub fn retrieve_y_bounds_from_plot_data(
    ...
    for series in &plot_data.point_collection {
        ...
        let zoom_bounds = retrieve_y_bounds_from_collection_of_points(&only_in_bounds);
...
pub fn retrieve_y_bounds_from_collection_of_points(
    series_data: &Vec<(f64, DataValue)>,
) -> ZoomBound {
    let mut min: f64 = MAX;
    let mut max: f64 = MIN;
    for collection in series_data {
        let as_maybe_float = interpret_datavalue_as_float(&collection.1);
...
pub fn interpret_datavalue_as_float(value: &DataValue) -> Option<f64> {
    match value {
...
pub fn skip_outside_of_bound(
    ...
    let filtered_by_y: Vec<(f64, DataValue)> = filtered_by_x
        .iter()
        .filter(|collection| {
            let val_as_float = interpret_datavalue_as_float(&collection.1);
