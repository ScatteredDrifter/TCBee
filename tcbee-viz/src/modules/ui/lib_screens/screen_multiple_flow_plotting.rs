...
use iced::{
    widget::{scrollable, text, Column, Row, Rule, Space},
...
use crate::modules::{
    backend::{
        plot_data_preprocessing::filter_and_prepare_string_from_series,
        struct_tcp_flow_wrapper::TcpFlowWrapper,
    },
    ui::{
        lib_styling::app_style_settings::{
            CHART_HEIGHT, HORIZONTAL_LINE_PRIMARY_HEIGHT, SCROLLABLE_TEXT_WINDOWS_SIZE,
        ...
        lib_widgets::{
            app_widgets::{
                ...
                display_current_mouse_position, display_empty_screen_no_data,
                display_line_series_toggle, display_prepared_string, display_split_graph_selector,
                display_zoom_sliders, generate_legends_for_charts, generate_padded_layout,
            },
            lib_graphs::{
                multiple_charts_flow_series_data::create_multiple_charts,
                single_chart_processed_plot_data::MessageCreator,
                struct_zoom_bounds::{
                    generate_zoom_bounds_from_coordinates_in_data,
                    retrieve_default_zoom_for_one_flow, retrieve_default_zoom_for_two_flows,
                    retrieve_max_series_bound_of_two_flows, ZoomBound, ZoomBound2D,
                },
            },
        },
    },
...
impl ScreenMultiFlowPlotting {
    ...
    pub fn update(&mut self, message: MessageMultiFlowPlotting) {
        match message {
            MessageMultiFlowPlotting::FlowOneSelected(id) => {
                ...
                self.zoom_boundaries = Some(retrieve_default_zoom_for_two_flows(
                    &self.application_settings,
                    &self.first_tcp_flow,
            ...
            MessageMultiFlowPlotting::SliderZoomLX(lower_bound) => {
                let new_x_bound = ZoomBound {
                    lower: lower_bound,
                    upper: self.zoom_boundaries.clone().unwrap_or_default().x.upper,
                };
            ...
            MessageMultiFlowPlotting::MouseEvent(event, point) => {
                ...
                match event {
                    ...
                    iced::mouse::Event::ButtonReleased(iced::mouse::Button::Left) => {
                        ...
                        // update zoom
                        let data_reference =
                            self.plot_data.as_mut().expect("no plotting data avail");
                        let new_zoom =
                            generate_zoom_bounds_from_coordinates_in_data(&data_reference);
    ...
    fn display_sidebar(&self) -> Element<'_, MessageMultiFlowPlotting> {
        ...
        let content = Column::new()
            ...
            .push(flow_selection)
            .push(
                scrollable(lower_sidebar)
                    .height(Length::FillPortion(HEIGHT_GRAPH_SETTINGS_PORTION)),
            );
    ...
    fn generate_chart_view(&self) -> Element<'_, MessageMultiFlowPlotting> {
        let generate_top_info: bool = self.render_chart
            && self.first_tcp_flow.selected_series.is_some()
            && self.second_tcp_flow.selected_series.is_some();
    ...
    fn generate_chart(&self) -> Element<'_, MessageMultiFlowPlotting> {
        let read_settings = self.application_settings.read().unwrap();
        if !self.render_chart
            || (self.first_tcp_flow.selected_series.is_none()
                && self.second_tcp_flow.selected_series.is_none())
        {
...
impl Screen for ScreenMultiFlowPlotting {
    ...
    fn content(&self) -> Element<'_, Self::Message> {
        ...
        let content: Element<'_, MessageMultiFlowPlotting> = Row::new()
            ...
            .into();
        let padded_content: Element<'_, MessageMultiFlowPlotting> =
            generate_padded_layout(20).push(content).into();
